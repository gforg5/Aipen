...
const PROJECTS_STORAGE_KEY = 'AIPEN_PRO_STORAGE_FINAL_V12';
    ...
    <>
        ...
        <div className="flex items-center gap-3">
          {activeProject && currentStep !== AppState.VIEWER && (
            <button onClick={() => handleNav(AppState.VIEWER)} className="hidden sm:flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl hover:bg-indigo-700 transition-all shadow-xl font-black text-[9px] uppercase tracking-widest">
              <i className="fas fa-book-open text-[8px]"></i> Read Book
            </button>
          )}
          <button onClick={() => setIsMenuOpen(true)} className="lg:hidden w-10 h-10 rounded-lg flex items-center justify-center bg-slate-50 text-slate-900 transition-all active:scale-95"><i className="fas fa-bars-staggered"></i></button>
        </div>
      </header>
      ...
      <div className={`fixed inset-0 w-full h-full bg-white z-[300] lg:hidden transition-all duration-500 flex flex-col ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full pointer-events-none'}`}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-50">
          ...
          <button onClick={() => setIsMenuOpen(false)} className="w-10 h-10 rounded-full bg-slate-100 text-slate-900 flex items-center justify-center active:scale-90 transition-all"><i className="fas fa-times"></i></button>
        </div>
        <div className="flex-1 flex flex-col justify-center px-10 gap-4">
          {[{ label: 'Studio', step: AppState.HOME }, { label: 'Developer Info', step: AppState.DEVELOPER }].map((item) => (
...
// Footer component fix: Defined the missing Footer component to avoid the ReferenceError on line 408
const Footer: React.FC = () => (
  <footer className="py-12 px-6 border-t border-slate-50 text-center no-print bg-white mt-auto w-full">
    ...
    <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">Powered by Neural Synthesis Engine v12.0</p>
  </footer>
);
...
const VisualPlaceholder: React.FC<{desc: string, genre: string, onReplace: (desc: string, b64: string) => void}> = ({desc, genre, onReplace}) => {
  ...
  return (
    <div className="my-16 p-10 bg-slate-50 rounded-[40px] border border-slate-100 text-center no-print animate-reveal-skew">
      <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Neural Synthesis Required</div>
      ...
      <button onClick={generate} disabled={loading} className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl disabled:opacity-50 hover:scale-105 active:scale-95 transition-all">
        ...
        {loading ? "Materializing..." : "Generate Illustration"}
      </button>
    </div>
  )
}
...
const App: React.FC = () => {
  ...
  const startOutline = async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to architect blueprint. Verify API key.");
  ...
  const startWriting = async () => {
    ...
    try {
      for (let i = 0; i < updatedOutline.length; i++) {
        setProgress({ currentChapter: i + 1, totalChapters: updatedOutline.length, message: `Synthesizing Segment: ${updatedOutline[i].title}` });
      ...
      <main className="flex-1 flex flex-col items-center w-full relative">
        {error && (
          <div className="fixed top-24 z-[110] w-[90%] max-w-xl bg-slate-900 text-white px-8 py-4 rounded-2xl flex items-center justify-between no-print shadow-2xl animate-fade-in-up">
            ...
            <button onClick={() => setError(null)} className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 transition-all flex items-center justify-center"><i className="fas fa-times"></i></button>
          </div>
        )}
          ...
          <div className="w-full animate-fade-in-up no-print">
              ...
              <div className="grid lg:grid-cols-12 gap-12 items-center">
                <div className="lg:col-span-7 space-y-10">
                  <div className="space-y-6">
                    <div className="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-100 rounded-full animate-text-float">
                      <span className="w-2 h-2 bg-indigo-500 rounded-full animate-pulse"></span>
                      <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">v12.0 Pro Engine</span>
                    </div>
                    <h2 className="text-5xl md:text-8xl font-black text-slate-900 serif-text tracking-tighter leading-none animate-reveal-skew">
                      Architect <br/>
                      <span className="text-indigo-600 italic">Masterpieces.</span>
                    </h2>
                  </div>

                  <div className="bg-white p-8 md:p-12 rounded-[48px] border border-slate-100 shadow-2xl space-y-8 transition-all hover:shadow-indigo-100/50">
                    <div className="grid sm:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        ...
                        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="AgenticAI"
                          className="w-full px-6 py-5 rounded-2xl border border-slate-100 bg-slate-50 text-slate-900 focus:ring-4 focus:ring-indigo-100 focus:bg-white outline-none font-black serif-text text-2xl transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Author</label>
                        <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="SMA"
                          className="w-full px-6 py-5 rounded-2xl border border-slate-100 bg-slate-50 text-slate-900 focus:ring-4 focus:ring-indigo-100 transition-all font-bold" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Volume (Pages)</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Classification</label>
                    ...
                    <button onClick={startOutline} disabled={loading} className="w-full py-7 bg-slate-900 text-white rounded-[24px] font-black transition-all shadow-xl disabled:bg-slate-200 uppercase text-[12px] tracking-[0.4em] flex items-center justify-center gap-4 btn-killer active:scale-95">
                      {loading ? <i className="fas fa-circle-notch fa-spin"></i> : <i className="fas fa-pen-nib"></i>}
                      {loading ? "Architecting..." : "Architect Volume"}
                    </button>
                  </div>
                </div>
                <div className="lg:col-span-5 hidden lg:block"><div className="killer-perspective"><div className="killer-tilt rounded-[48px] shadow-3xl rotate-3 border-8 border-white"><img src="https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&q=80&w=1200" alt="Hero" /></div></div></div>
              </div>
            </section>
            ...
            <section className="bg-slate-50 py-24 px-6">
              <div className="max-w-7xl mx-auto w-full">
                ...
                {projects.length === 0 ? (
                  <div className="py-24 bg-white border border-slate-100 rounded-[64px] flex flex-col items-center text-center shadow-sm">
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-10">
                    {projects.map(project => (
                      <div key={project.id} onClick={() => loadProject(project)} className="group bg-white p-10 rounded-[48px] border border-slate-100 hover-card cursor-pointer relative transition-all">
                        <button onClick={(e) => { e.stopPropagation(); deleteProject(project.id); }} className="absolute top-8 right-8 text-slate-200 hover:text-red-500 z-10 p-2 transition-colors"><i className="fas fa-trash-alt"></i></button>
                        <div className="space-y-8">
                           <div className="w-16 h-16 bg-slate-50 rounded-[20px] flex items-center justify-center group-hover:bg-slate-900 group-hover:text-white transition-all shadow-sm"><i className="fas fa-book text-3xl"></i></div>
                           <div><h4 className="text-3xl font-black text-slate-900 serif-text line-clamp-1 group-hover:text-indigo-600 transition-colors">{project.title}</h4><div className="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-2">{project.genre} • {new Date(project.createdAt).toLocaleDateString()}</div></div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {step === AppState.OUTLINING && (
          <div className="py-20 animate-fade-in-up w-full max-w-5xl px-6 no-print">
             <div className="bg-white p-14 rounded-[64px] border border-slate-100 shadow-3xl space-y-12">
                <div className="flex justify-between items-center"><h2 className="text-5xl font-black serif-text text-slate-900 uppercase">Architecture Blueprint</h2><button onClick={() => setStep(AppState.HOME)} className="text-[10px] font-black uppercase text-slate-400 hover:text-slate-900">Cancel</button></div>
                <div className="grid md:grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-4 scroll-smooth">
                   {currentBook?.outline.map((ch, idx) => (
                      <div key={ch.id} className="p-8 bg-slate-50 rounded-3xl flex flex-col gap-3 border border-slate-100 transition-all hover:bg-white hover:shadow-xl group">
                         <span className="font-bold text-slate-900 text-xl group-hover:text-indigo-600 transition-colors">{idx + 1}. {ch.title}</span>
                ...
                <button onClick={startWriting} className="w-full py-8 bg-indigo-600 text-white rounded-[28px] font-black uppercase tracking-[0.3em] shadow-2xl hover:scale-[1.02] active:scale-95 transition-all text-sm">Initiate Neural Drafting Cycle</button>
             </div>
          </div>
        )}
        ...
        {step === AppState.WRITING && (
           <div className="py-48 text-center animate-fade-in-up flex flex-col items-center w-full px-6 no-print">
              ...
              <h2 className="text-5xl font-black serif-text text-slate-900 uppercase mb-6 tracking-tight">Synthesizing Manuscript</h2>
              <div className="h-2 w-24 bg-indigo-500 mx-auto mb-10 rounded-full"></div>
              <p className="text-indigo-600 font-bold italic text-2xl animate-pulse">{progress.message}</p>
              <div className="w-full max-w-lg h-3 bg-slate-50 rounded-full mx-auto overflow-hidden border border-slate-100 mt-14 shadow-inner">
                 <div className="h-full bg-indigo-600 transition-all duration-1000 shadow-xl" style={{ width: `${(progress.currentChapter / progress.totalChapters) * 100}%` }}></div>
              </div>
           </div>
        )}
          ...
          <>
            <div className="hidden print:block w-full">
              <div className="book-page flex flex-col items-center justify-center text-center" style={{ breakAfter: 'page' }}>
                 <div className="text-[24px] font-black tracking-[1.8em] uppercase text-slate-900 mb-24">O F F I C I A L  B O O K</div>
                 ...
                 <div className="mt-auto text-[12px] font-black text-slate-300 uppercase tracking-widest">AiPen Studio v12.0 Pro Edition</div>
              </div>
              {currentBook.outline.map((ch, idx) => (
            ...
            <div className="w-full animate-fade-in-up flex flex-col items-center px-6 no-print">
              <div className="fixed bottom-10 left-1/2 -translate-x-1/2 md:left-24 md:top-1/2 md:-translate-y-1/2 flex md:flex-col gap-10 z-[120] bg-white/90 backdrop-blur-3xl p-6 rounded-[50px] shadow-3xl border border-white/40">
                 <button disabled={activeChapterIndex === 0} onClick={() => { setActiveChapterIndex(p => p - 1); window.scrollTo({top: 0, behavior: 'smooth'}); }}
                  className="w-18 h-18 md:w-28 md:h-28 bg-white border border-slate-200 rounded-full flex items-center justify-center text-slate-300 hover:text-indigo-600 hover:scale-110 active:scale-90 transition-all disabled:opacity-20"><i className="fas fa-chevron-left text-3xl"></i></button>
              ...
              <div className="w-full max-w-5xl space-y-16 mb-40 mt-12">
                 <div className="flex flex-col sm:flex-row justify-between items-center gap-10 bg-white p-12 rounded-[56px] border border-slate-100 shadow-2xl">
                   ...
                   <div className="flex gap-4 w-full sm:w-auto">
                      <button onClick={() => setStep(AppState.HOME)} className="px-10 py-6 bg-slate-50 text-slate-900 rounded-[24px] font-black text-[11px] uppercase tracking-widest hover:bg-slate-100 transition-all">Studio</button>
                      <button onClick={() => window.print()} className="px-12 py-6 bg-slate-900 text-white rounded-[24px] font-black text-[11px] uppercase tracking-widest shadow-2xl flex items-center gap-4 hover:scale-105 transition-all"><i className="fas fa-file-pdf"></i> Export Manuscript</button>
                   </div>
                 </div>
                 
                 <div className="bg-white rounded-[80px] shadow-[0_80px_160px_-40px_rgba(0,0,0,0.1)] p-14 md:p-36 border border-slate-50 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-indigo-500 to-purple-500"></div>
                    {activeChapterIndex === 0 && (
                      <div className="mb-48 text-center border-b border-slate-50 pb-40 space-y-16 animate-reveal-skew">
                         <div className="text-[20px] font-black tracking-[1.6em] uppercase text-slate-900 opacity-80">O F F I C I A L  B O O K</div>
                    ...
                    <div className="flex justify-between items-center mb-20 border-b border-slate-50 pb-10">
                       ...
                       <div className="text-[12px] font-black text-slate-300 uppercase italic tracking-widest">Architect Draft v12.0 Pro</div>
                    </div>
           ...
           <div className="w-full max-w-5xl px-6 py-24 animate-fade-in-up no-print">
              <div className="bg-slate-900 p-16 md:p-24 rounded-[80px] shadow-3xl text-center space-y-16 relative overflow-hidden">
                 ...
                 <div className="w-56 h-56 mx-auto rounded-[60px] border-[10px] border-white/10 overflow-hidden shadow-2xl transition-transform hover:scale-110 duration-1000 rotate-3">
                    <img src="https://github.com/gforg5/Nano-Lens/blob/main/1769069098374.png?raw=true" className="w-full h-full object-cover grayscale brightness-125" alt="SMA" />
                 </div>
                 <div className="space-y-6">
                    <h2 className="text-6xl font-black text-white serif-text uppercase tracking-tight">Sayed Mohsin Ali</h2>
                    <p className="text-indigo-400 text-2xl font-medium italic serif-text max-w-2xl mx-auto">"Architecting the future of intelligent storytelling through high-fidelity neural synthesis."</p>
                 </div>
                 <div className="flex justify-center gap-8">
                    <a href="https://github.com/gforg5" target="_blank" className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-white hover:bg-white hover:text-slate-900 transition-all shadow-xl"><i className="fab fa-github text-3xl"></i></a>
                    <a href="https://www.linkedin.com/in/sayed-mohsin-ali-924b8926b" target="_blank" className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-white hover:bg-[#0077B5] hover:text-white transition-all shadow-xl"><i className="fab fa-linkedin-in text-3xl"></i></a>
                 </div>
